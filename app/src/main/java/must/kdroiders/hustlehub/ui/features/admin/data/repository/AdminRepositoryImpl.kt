...
class AdminRepositoryImpl @Inject constructor(
    ...
    override suspend fun getAnalytics(): Result<AdminAnalytics> = runCatching {
    ...
    override suspend fun getUsers(page: Int, size: Int): Result<List<AdminUserItem>> = runCatching {
    ...
    override suspend fun suspendUser(id: String, reason: String): Result<Unit> = runCatching {
    ...
    override suspend fun unsuspendUser(id: String, reason: String): Result<Unit> = runCatching {
        val response = apiService.unsuspendUser(id, AdminActionRequestDto(reason))
        if (!response.success) throw IllegalStateException(response.message)
    }

    override suspend fun verifyPro(id: String, reason: String): Result<Unit> = runCatching {
        val response = apiService.verifyPro(id, AdminActionRequestDto(reason))
        if (!response.success) throw IllegalStateException(response.message)
    }

    override suspend fun revokePro(id: String, reason: String): Result<Unit> = runCatching {
        val response = apiService.revokePro(id, AdminActionRequestDto(reason))
        if (!response.success) throw IllegalStateException(response.message)
    }

    override suspend fun delistService(id: String, reason: String): Result<Unit> = runCatching {
        val response = apiService.delistService(id, AdminActionRequestDto(reason))
        if (!response.success) throw IllegalStateException(response.message)
    }

    override suspend fun relistService(id: String, reason: String): Result<Unit> = runCatching {
        val response = apiService.relistService(id, AdminActionRequestDto(reason))
        if (!response.success) throw IllegalStateException(response.message)
    }

    override suspend fun getReports(status: String?, page: Int, size: Int): Result<List<AdminReportItem>> = runCatching {
    ...
    override suspend fun resolveReport(id: String, reason: String): Result<Unit> = runCatching {
        val response = apiService.resolveReport(id, AdminActionRequestDto(reason))
        if (!response.success) throw IllegalStateException(response.message)
    }

    override suspend fun dismissReport(id: String, reason: String): Result<Unit> = runCatching {
        val response = apiService.dismissReport(id, AdminActionRequestDto(reason))
        if (!response.success) throw IllegalStateException(response.message)
    }

    override suspend fun getAuditLogs(targetType: String, targetId: String): Result<List<AdminAuditLogItem>> = runCatching {
